/**
 * buoy bot - Manage PR bot integrations
 *
 * buoy bot github    - GitHub App integration
 * buoy bot gitlab    - GitLab integration (coming soon)
 */
...
export function createBotCommand(): Command {
  ...
  cmd
    .description('Manage PR bot integrations')
...
function createGitHubCommand(): Command {
  ...
  cmd
    ...
    .addCommand(createGitHubStatusCommand())
    .addCommand(createGitHubConnectCommand())
    .addCommand(createGitHubDisconnectCommand());
...
function createGitHubStatusCommand(): Command {
  return new Command('status')
    ...
    .action(async (options) => {
      ...
      try {
        ...
        header('GitHub Bot Integration');
        newline();

        if (installations.length === 0) {
          info('No GitHub App installed');
          newline();
          info('Run `buoy bot github connect` to install the Buoy GitHub App');
          info('This enables:');
...
function createGitHubConnectCommand(): Command {
  return new Command('connect')
    ...
    .action(async () => {
      ...
      try {
        ...
        newline();
        info('After installation, run `buoy bot github status` to verify.');
      } catch (err) {
...
function createGitHubDisconnectCommand(): Command {
    ...
    .action(async (account, options) => {
      ...
      try {
        ...
        if (!options.all && account) {
          ...
          for (const i of installations) {
            info(`  buoy bot github disconnect ${i.accountLogin}`);
          }
          info('  buoy bot github disconnect --all');
          process.exit(1);
        }
...
function createGitLabCommand(): Command {
  ...
  cmd
    ...
    .action(() => {
      ...
      newline();
      info('In the meantime, you can use the CLI directly in GitLab CI:');
      newline();
