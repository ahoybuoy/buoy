...
import { ProjectDetector, type DetectedProject, detectMonorepoConfig, expandPatternsForMonorepo } from "../detect/index.js";
 ...
 */
async function runSmartDock(options: { yes?: boolean; json?: boolean }) {
  ...
  if (options.json) {
    ...
    if (allDone) {
      ...
      console.log("");
      if (!results.configCreated) console.log(`    Run ${chalk.cyan("buoy dock config")}`);
      if (!results.agentsSetup) console.log(`    Run ${chalk.cyan("buoy dock agents")}`);
      if (!results.hooksSetup) console.log(`    Run ${chalk.cyan("buoy dock hooks")}`);
    }
  }
 ...
 */
async function runConfigDock(options: { force?: boolean; yes?: boolean; skipDetect?: boolean }) {
  const cwd = process.cwd();
  ...
  try {
    ...
    if (options.dryRun) {
      ...
      const skillResult = await exportService.export(
        { tokens: scanResult.tokens, components: scanResult.components, drifts: diffResult.drifts, projectName },
        { sections: ["tokens", "components", "patterns", "anti-patterns"], outputPath: skillPath }
      );
    ...
    if (options.json) {
      ...
      console.log("");
      if (results.skillCreated) console.log(`  ${chalk.green("✓")} Created skill files`);
      if (results.contextUpdated) console.log(`  ${chalk.green("✓")} Updated CLAUDE.md`);
 ...
 */
async function runSkillsDock(options: { output?: string; global?: boolean; dryRun?: boolean; json?: boolean }) {
  const cwd = process.cwd();
  if (options.json) setJsonMode(true);
  ...
  try {
    ...
    if (options.global) outputPath = join(homedir(), ".claude", "skills", projectName);
    ...
    const result = await exportService.export(
      { tokens: scanResult.tokens, components: scanResult.components, drifts: diffResult.drifts, projectName },
      { sections: ["tokens", "components", "patterns", "anti-patterns"], outputPath }
    );
 ...
 */
async function runContextDock(options: { output?: string; stdout?: boolean; detail?: string; json?: boolean }) {
  const cwd = process.cwd();
  if (options.json) setJsonMode(true);
  ...
  try {
    ...
    const contextResult = generateContext(
      { tokens: scanResult.tokens, components: scanResult.components, drifts: diffResult.drifts, projectName },
      { detailLevel: (options.detail as "minimal" | "standard" | "comprehensive") || "standard" }
    );
...
async function loadOrBuildConfig(cwd: string): Promise<{ config: BuoyConfig; projectName: string }> {
...
function installClaudeCommands(dryRun = false): { installed: string[]; alreadyExisted: string[] } {
  ...
  if (!dryRun && !existsSync(commandsDir)) mkdirSync(commandsDir, { recursive: true });
...
function getSourceKey(name: string): string | null {
  if (["react", "nextjs", "remix", "gatsby", "react-native", "expo", "preact", "solid"].includes(name)) return "react";
...
async function showFrameworksAndPlugins(frameworks: Array<{ name: string; scanner?: string; plugin?: string }>, autoInstall?: boolean): Promise<void> {
  ...
  if (builtIn.length > 0) {
    ...
    for (const fw of builtIn) console.log(`  ${chalk.green("✓")} ${chalk.cyan(fw.name)}`);
    console.log("");
  }
