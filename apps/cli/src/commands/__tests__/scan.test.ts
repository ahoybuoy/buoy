...
describe('scan command', () => {
  ...
  describe('argument parsing', () => {
    it('creates scan command with correct options', () => {
      ...
      expect(cmd.description()).toBe('Scan sources for components and tokens');
    ...
    it('parses --source option with multiple sources', async () => {
      ...
      (scanners.TokenScanner as unknown as ReturnType<typeof vi.fn>).mockImplementation(
        () => mockTokenScanner
      );

      const program = createTestProgram();
      await program.parseAsync(['node', 'test', 'scan', '-s', 'react', '-s', 'tokens']);
  ...
  describe('source auto-detection', () => {
    it('scans enabled sources from config when no --source specified', async () => {
      ...
      (scanners.VueComponentScanner as unknown as ReturnType<typeof vi.fn>).mockImplementation(
        () => mockVueScanner
      );
  ...
  describe('output formats', () => {
    ...
    it('shows verbose output when --verbose flag is provided', async () => {
      ...
      expect(reporters.info).toHaveBeenCalledWith(expect.stringContaining('Using config:'));
    });
  });
  ...
  describe('scanner configuration', () => {
    ...
    it('passes designSystemPackage to react scanner', async () => {
      ...
      (scanners.ReactComponentScanner as unknown as ReturnType<typeof vi.fn>).mockImplementation(
        (config: { designSystemPackage: string }) => {
    ...
    it('reports scanner errors in results', async () => {
      ...
      const mockReactScanner = {
        scan: vi.fn().mockResolvedValue({
          items: [createMockComponent('Button')],
          errors: [
            { file: 'src/Broken.tsx', message: 'Syntax error', code: 'PARSE_ERROR' },
          ],
          ...
          items: [
            {
              ...
              source: { type: 'vue', path: 'src/Modal.vue', exportName: 'Modal', line: 1 },
            },
          ],
          errors: [],
          stats: { filesScanned: 1, itemsFound: 1, duration: 50 },
  ...
  describe('framework-specific scanners', () => {
    it('scans svelte source with correct scanner', async () => {
      mockLoadConfig.mockResolvedValue({
        config: createMockConfig({
          sources: {
            svelte: { enabled: true, include: ['src/**/*.svelte'], exclude: [] },
          },
        }),
        configPath: '/test/buoy.config.js',
      });
      ...
      (scanners.SvelteComponentScanner as unknown as ReturnType<typeof vi.fn>).mockImplementation(
        () => mockScanner
      );
    ...
    it('scans angular source with correct scanner', async () => {
      mockLoadConfig.mockResolvedValue({
        config: createMockConfig({
          sources: {
            angular: { enabled: true, include: ['src/**/*.component.ts'], exclude: [] },
          },
        }),
        configPath: '/test/buoy.config.js',
      });
      ...
      (scanners.AngularComponentScanner as unknown as ReturnType<typeof vi.fn>).mockImplementation(
        () => mockScanner
      );
    ...
    it('scans webcomponent source with correct scanner', async () => {
      mockLoadConfig.mockResolvedValue({
        config: createMockConfig({
          sources: {
            webcomponent: { enabled: true, include: ['src/**/*.ts'], exclude: [], framework: 'auto' },
          },
        }),
        configPath: '/test/buoy.config.js',
      });
      ...
      (scanners.WebComponentScanner as unknown as ReturnType<typeof vi.fn>).mockImplementation(
        () => mockScanner
      );
    ...
    it('scans templates with correct scanner and type', async () => {
      ...
      (scanners.TemplateScanner as unknown as ReturnType<typeof vi.fn>).mockImplementation(
        (config: { templateType: string }) => {
  ...
  describe('JSON output structure', () => {
    it('outputs valid JSON with expected structure', async () => {
      ...
      const mockReactScanner = {
        scan: vi.fn().mockResolvedValue({
          items: [mockComponent],
          errors: [{ file: 'src/Broken.tsx', message: 'Parse error', code: 'PARSE_ERROR' }],
          stats: { filesScanned: 2, itemsFound: 1, duration: 100 },
        }),
      };
