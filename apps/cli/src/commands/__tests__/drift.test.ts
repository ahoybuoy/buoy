...
describe('drift command', () => {
  ...
  describe('command structure', () => {
    ...
    it('check subcommand has correct options', () => {
      ...
      const optionNames = checkCmd!.options.map(o => o.long?.replace('--', ''));
  ...
  describe('drift check - basic execution', () => {
    beforeEach(() => {
      ...
      (analysis.SemanticDiffEngine as unknown as ReturnType<typeof vi.fn>).mockImplementation(
        () => mockEngine
      );
    });
    ...
    it('displays summary statistics', async () => {
      ...
      expect(reporters.keyValue).toHaveBeenCalledWith('Components scanned', '2');
    ...
    it('shows warning when critical issues exist', async () => {
      ...
      expect(reporters.warning).toHaveBeenCalledWith('1 critical issues require attention.');
    });
  });
  ...
  describe('drift check - output formats', () => {
    ...
    it('outputs markdown when --markdown flag is provided', async () => {
      const program = createTestProgram();
      await program.parseAsync(['node', 'test', 'drift', 'check', '--markdown']);
    ...
    it('JSON output includes summary with counts', async () => {
      ...
      const data = formatJsonCall![0] as { drifts: DriftSignal[]; summary: Record<string, number> };
  ...
  describe('drift check - severity filtering', () => {
    ...
    it('filters drifts by --severity critical', async () => {
      ...
      const program = createTestProgram();
      await program.parseAsync(['node', 'test', 'drift', 'check', '--severity', 'critical']);
    ...
    it('filters drifts by --severity warning (includes critical)', async () => {
      ...
      const program = createTestProgram();
      await program.parseAsync(['node', 'test', 'drift', 'check', '--severity', 'warning']);
    ...
    it('shows all drifts with --severity info', async () => {
      ...
      const program = createTestProgram();
      await program.parseAsync(['node', 'test', 'drift', 'check', '--severity', 'info']);
  ...
  describe('drift check - type filtering', () => {
    ...
    it('filters drifts by --type', async () => {
      ...
      const program = createTestProgram();
      await program.parseAsync(['node', 'test', 'drift', 'check', '--type', 'hardcoded-value']);
    ...
    it('applies ignore rules from config', async () => {
      mockLoadConfig.mockResolvedValue({
        config: createMockConfig({
          ...
          drift: {
            ignore: [
              { type: 'hardcoded-value', pattern: 'Legacy.*' },
            ],
            severity: {},
          },
        }),
  ...
  describe('drift check - error handling', () => {
    ...
    it('handles scanner failure', async () => {
      mockLoadConfig.mockResolvedValue({
        config: createMockConfig({
          sources: {
            react: { enabled: true, include: ['src/**/*.tsx'], exclude: [] },
  ...
  describe('drift explain subcommand', () => {
    it('shows not implemented message', async () => {
      const program = createTestProgram();
      await program.parseAsync(['node', 'test', 'drift', 'explain', 'drift-123']);

      expect(reporters.info).toHaveBeenCalledWith('Claude integration not yet implemented.');
      expect(reporters.info).toHaveBeenCalledWith('To explain drift: drift-123');
    });
  });

  describe('drift resolve subcommand', () => {
    it('marks drift as resolved with default resolution type', async () => {
      const program = createTestProgram();
      await program.parseAsync(['node', 'test', 'drift', 'resolve', 'drift-456']);

      expect(reporters.info).toHaveBeenCalledWith('Marking drift drift-456 as fixed');
      expect(reporters.success).toHaveBeenCalledWith(
        'Drift resolved (note: persistence not yet implemented)'
      );
    });

    it('accepts custom resolution type', async () => {
      const program = createTestProgram();
      await program.parseAsync([
        'node', 'test', 'drift', 'resolve', 'drift-789',
        '--resolution', 'ignored',
      ]);

      expect(reporters.info).toHaveBeenCalledWith('Marking drift drift-789 as ignored');
    });

    it('accepts resolution message', async () => {
      const program = createTestProgram();
      await program.parseAsync([
        'node', 'test', 'drift', 'resolve', 'drift-abc',
        '--message', 'This is intentional for legacy support',
      ]);

      expect(reporters.info).toHaveBeenCalledWith(
        'Reason: This is intentional for legacy support'
      );
    });
  });
  ...
  describe('drift check - scanner configuration', () => {
    ...
    it('passes correct scanner configuration', async () => {
      ...
      (scanners.ReactComponentScanner as unknown as ReturnType<typeof vi.fn>).mockImplementation(
        (config: { include: string[]; exclude: string[]; designSystemPackage: string }) => {
  ...
  describe('summary calculation', () => {
    ...
    it('JSON output includes correct summary', async () => {
      ...
      const formatJsonCall = vi.mocked(formatters.formatJson).mock.calls[0];
      const data = formatJsonCall![0] as { summary: { critical: number; warning: number; info: number } };
