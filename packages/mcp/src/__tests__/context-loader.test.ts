...
describe('Context Loader', () => {
  ...
  describe('loadDesignSystemContext', () => {
    ...
    it('loads tokens from ai-context format', async () => {
      vi.mocked(existsSync).mockImplementation((path) => {
        const p = String(path);
        return p.includes('package.json') || p.includes('tokens-ai-context.json');
      });
      vi.mocked(readFileSync).mockImplementation((path) => {
        const p = String(path);
        if (p.includes('package.json')) {
  ...
  describe('getTokensByCategory', () => {
    ...
    it('returns empty array for unknown category', () => {
      const context = {
        tokens: [{ name: 'primary', value: '#2563EB', category: 'color' as const }],
        components: [],
        patterns: [],
        antiPatterns: [],
        projectName: 'Test',
  ...
  describe('findComponent', () => {
    const context = {
      tokens: [],
      components: [
        { name: 'Button', framework: 'react', props: ['variant', 'size'], path: '' },
        { name: 'Card', framework: 'react', props: ['title'], path: '' },
        { name: 'Input', framework: 'react', props: ['type', 'placeholder'], path: '' },
      ],
      patterns: [],
      antiPatterns: [],
      projectName: 'Test',
  ...
  describe('searchComponents', () => {
    const context = {
      tokens: [],
      components: [
        { name: 'Button', framework: 'react', props: ['variant', 'size'], description: 'Clickable button', path: '' },
        { name: 'IconButton', framework: 'react', props: ['icon'], description: 'Button with icon', path: '' },
        { name: 'Card', framework: 'react', props: ['title'], description: 'Content container', path: '' },
        { name: 'Input', framework: 'react', props: ['type'], description: 'Form input field', path: '' },
      ],
      patterns: [],
      antiPatterns: [],
      projectName: 'Test',
