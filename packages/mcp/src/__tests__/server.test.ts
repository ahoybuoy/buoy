...
vi.mock('../context-loader.js', () => ({
  ...
  findComponent: vi.fn((ctx, name) =>
    ctx.components.find((c: { name: string }) => c.name.toLowerCase() === name.toLowerCase())
  ),
  searchComponents: vi.fn((ctx, useCase) => {
    const keywords = useCase.toLowerCase().split(/\s+/);
    return ctx.components.filter((c: { name: string; description?: string }) => {
...
describe('Server Tool Logic', () => {
  ...
  describe('resolve_token logic', () => {
    ...
    it('finds exact match', () => {
      const value = '#2563EB';
      const match = tokens.find(t => t.value.toLowerCase() === value.toLowerCase());
